...
namespace isic {
    ...
    OtaModule::OtaModule(OtaService &otaService, EventBus &bus) : m_ota(otaService), m_bus(bus) {
    ...
    void OtaModule::start() {
        ...
        if (const auto running= m_ota.getRunningPartition(); !running.label.empty()) {
            LOG_INFO(OTA_MODULE_TAG, "  Running: %s (v%s)", running.label.c_str(), running.appVersion.empty() ? "unknown" : running.appVersion.c_str());
        }

        if (const auto next = m_ota.getNextUpdatePartition(); !next.label.empty()) {
    ...
    void OtaModule::handleConfigUpdate(const AppConfig &config) {
        ...
        LOG_DEBUG(OTA_MODULE_TAG, "Config updated: enabled=%s, autoCheck=%s, autoUpdate=%s", enabled ? "yes" : "no", config.ota.autoCheck ? "yes" : "no", config.ota.autoUpdate ? "yes" : "no");
    }
    ...
    bool OtaModule::parseOtaCommand(const std::string &payload, OtaCommand &cmd) {
        ...
        if (const auto err = deserializeJson(doc, payload); err) {
        ...
        const auto *actionStr{root["action"] | ""};
    ...
    void OtaModule::onOtaStateChanged(const OtaStateChangedEvent &event) {
        ...
        LOG_INFO(OTA_MODULE_TAG, "OTA state: %s -> %s", toString(oldState), toString(newState));
    ...
    void OtaModule::onOtaProgress(const OtaProgressEvent &event) {
        ...
        if (event.percent >= lastLoggedPercent + 10 || event.percent == 100) {
            LOG_DEBUG(OTA_MODULE_TAG, "OTA progress: %u%% (%u/%u bytes)", unsigned{event.percent}, unsigned{event.bytesDownloaded}, unsigned{event.totalBytes});
    ...
    void OtaModule::onMqttMessage(const MqttMessageEvent &event) {
        ...
        if (auto result = handleOtaCommand(event.payload); !result.ok()) {
